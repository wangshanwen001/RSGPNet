...
class TransformerDecoder(nn.Module):
    ...
    def _get_rpb_matrix(self, reference_boxes, feat_size):
        ...
        is_compiling = getattr(torch.compiler, 'is_dynamo_compiling', lambda: False)()
        if is_compiling or self.compilable_stored_size == (
            H,
            W,
        ):
        ...
        try:
            ...
            # 如果不存在该属性，说明当前不是在编译模式，或者是旧版本 torch
            is_compiling = False
    ...
    def forward(
        ...
        for layer_idx, layer in enumerate(self.layers):
            ...
            )  # nq, bs, nlevel, 4 # [200, 1, 1, 4]

            query_sine_embed = gen_sineembed_for_position(
                reference_points_input[:, :, 0, :], self.d_model
            )  # nq, bs, d_model*2 # [200, 1, 512]

            # conditional query
            query_pos = self.ref_point_head(query_sine_embed)  # nq, bs, d_model # [200, 1, 256]
